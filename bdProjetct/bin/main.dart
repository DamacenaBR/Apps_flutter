...
var conn = await bd.iniciaConexao();

//await inserirDados1(conn, "Bruno", "ttf", 22);


  await listarDados(conn);
  //await deletarLinhas(conn);
  //await deletarTabela(conn);
}

Future<void> inserirDados1(PostgreSQLConnection conn, String nome, String email, int idade) async {
    await conn.open();
...
Future<void>  listarDados(PostgreSQLConnection conn)async{
  ...
  var r = await conn.mappedResultsQuery('SELECT * FROM Pessoa');
     for(int i=0; i< r.length;i++){
       print("###############");
     r[i].forEach((key,map) => map.forEach((key,values) => print("$key: ${values}" )) );
     print("\n");
   }
   await conn.close();
}
...
Future<void>  createTable(PostgreSQLConnection conn)async{
  ...
  await conn.open();
  await conn.execute('CREATE TABLE Pessoa (id SERIAL PRIMARY KEY, nome VARCHAR(50) NOT NULL, idade INTEGER, email VARCHAR(255))');
